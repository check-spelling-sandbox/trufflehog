	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				data:   []byte(fmt.Sprintf("You can find an apptivo secret %s within apptivo %s", secret, id)),
				verify: true,
			},
			want: []detectors.Result{
				{
		...
		},
		{
			...
			args: args{
				ctx:    context.Background(),
				data:   []byte(fmt.Sprintf("You can find an apptivo secret %s within but not valid apptivo %s", inactiveSecret, id)), // the secret would satisfy the regex but not pass validation
				verify: true,
			},
			want: []detectors.Result{
				{

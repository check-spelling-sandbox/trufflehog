	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				data:   []byte(fmt.Sprintf("You can find an etherscan secret %s within", secret)),
				verify: true,
			},
			want: []detectors.Result{
				{
		...
		},
		{
			...
			args: args{
				ctx:    context.Background(),
				data:   []byte(fmt.Sprintf("You can find an etherscan secret %s within but not valid", inactiveSecret)), // the secret would satisfy the regex but not pass validation
				verify: true,
			},
			want: []detectors.Result{
				{
